...
display = True
# alpha = 0.1
threshold = 20.3	#21  17    0.15   21   1.7
threshold1 = 21.3      #22.1   20   0.4   23.5   1.95
iou_threshold = 4.5#.5
# filewrite_img = False
...
def show_results(img,results):
	...
	for i in range(len(results)):
		...
		y = int(results[i][2])
		w = int(results[i][3])*2
		h1 = int(results[i][4])//1
                results[i][5] = results[i][5] * 3.8
		#x1 = int(results[0][1])
		#y1 = int(results[0][2])
	...
	#		goal.target_pose.pose.orientation.x = 3.0 #3 meters
	...
	#		goal.target_pose.pose.orientation.z = 1	
		...
		if imshow:
			cv2.rectangle(img_cp,(x-w,y-h),(x+w,y+h),(0,255,0),5) # 0 255 0 2
			#cv2.rectangle(img_cp,(x1-w1,y1-h1),(x1+w1,y1+h1),(0,255,255),5) # 0 255 0 2
			# cv2.rectangle(img_cp,(x-w,y-h-20),(x+w,y-h),(125,125,125),-5) # 125 125 125 -1
			cv2.putText(img_cp,'Detecting: ' + results[i][0] + ' - %.2f' % results[i][5]+'%',(x-w+5,y-h+400),cv2.FONT_HERSHEY_SIMPLEX,0.8,(0,0,255),2) # +5 -7 #0.5 000 1  # 
			#cv2.putText(img_cp,'Detecting: ' + results[0][0] + ' - %.2f' % results[0][5]+'%',(x1-w1+5,y1-h1+100),cv2.FONT_HERSHEY_SIMPLEX,0.8,(0,255,255),2) # +5 -7 #0.5 000 1  # 
		# if self.filewrite_txt :				
